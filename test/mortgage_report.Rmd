...
std_sched <- tibble(payment_no = 1:term, 
                    interest, 
                    principal, 
                    balance)
...
updated_sched %>%
  ...
  ggplot(aes(payment_no, amount, color = payment, fill = payment)) +
   geom_area(alpha = 0.25) +
   scale_color_manual(values = c("red", "darkgreen")) +
   scale_fill_manual(values = c("red", "darkgreen")) +
   geom_vline(xintercept = flip, linetype = "dotted", alpha = 0.5) +
   scale_y_continuous(labels = dollar_format(prefix = "$")) +
  theme_minimal() +
   labs(title = "Breakdown of Principal/Interest Payments", 
...
std_cum %>%
   ...
   geom_line(inherit.aes = FALSE, data = updated_cum,
             aes(payment_no, amount, color = payment), linetype = 2) +
   scale_color_manual(values = c("red", "darkgreen"),
